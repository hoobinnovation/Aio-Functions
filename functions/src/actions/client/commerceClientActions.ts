...
await ctx.db.transaction(async(tx:EntityManager)=>{const e=await tx.getRepository(CartItem).findOneBy({cartId:c.id,variantId});const nextQty=Number(e?.qty||0)+qty;if(v.stockQty<nextQty) throw new AppError('NOT_ENOUGH_STOCK','Requested quantity exceeds available stock');if(e) await tx.getRepository(CartItem).update({id:e.id},{qty:nextQty}); else await tx.getRepository(CartItem).save(tx.getRepository(CartItem).create({id:uuidv4(),cartId:c.id,productId:p.productId || v.productId,variantId,qty,unitPriceCents:v.priceCents}));});
return cartView(ctx);} 
export async function cartUpdateQty(ctx:ActionContext,p:any){const c=await cart(ctx);const qty=Math.max(1,Number(p.qty ?? p.quantity ?? 1));await ctx.db.transaction(async(tx:EntityManager)=>{const it=await tx.getRepository(CartItem).findOneBy({id:p.itemId,cartId:c.id});if(!it) throw new AppError('NOT_FOUND','Item not found');const v=it.variantId?await tx.getRepository(ProductVariant).findOneBy({id:it.variantId}):null;if(v && v.stockQty<qty) throw new AppError('NOT_ENOUGH_STOCK','Requested quantity exceeds available stock');await tx.getRepository(CartItem).update({id:it.id},{qty});});return cartView(ctx);} 
...
export async function notificationsList(ctx:ActionContext,payload:any={}){const q=normalizeListQueryInput(payload,{defaultPageSize:20,maxPageSize:200});const repo=ctx.db.getRepository(Notification);const [notifications,total,unreadCount]=await Promise.all([repo.find({where:{uid:ctx.uid!},order:{createdAt:'DESC' as any},take:q.limit,skip:q.offset}),repo.count({where:{uid:ctx.uid!}}),repo.count({where:{uid:ctx.uid!,isRead:false}})]);return {notifications,total,unreadCount,page:q.page,pageSize:q.pageSize,pagination:{page:q.page,pageSize:q.pageSize,total,hasMore:q.offset+notifications.length<total}};}
...
export async function loyaltyListTransactions(ctx:ActionContext,payload:any={}){const q=normalizeListQueryInput(payload,{defaultPageSize:20,maxPageSize:200});return {transactions:await ctx.db.getRepository(LoyaltyTransaction).find({where:{uid:ctx.uid!,storeId:ctx.storeId!},order:{createdAt:'DESC' as any},take:q.limit,skip:q.offset}),pagination:{page:q.page,pageSize:q.pageSize}};}
...
export async function walletHistory(ctx:ActionContext,payload:any={}){const q=normalizeListQueryInput(payload,{defaultPageSize:20,maxPageSize:200});return {transactions:await ctx.db.getRepository(WalletTransaction).find({where:{uid:ctx.uid!},order:{createdAt:'DESC' as any},take:q.limit,skip:q.offset}),pagination:{page:q.page,pageSize:q.pageSize}};}
...
export async function supportListTickets(ctx:ActionContext,payload:any={}){const q=normalizeListQueryInput(payload,{defaultPageSize:20,maxPageSize:200});return {tickets:await ctx.db.getRepository(SupportTicket).find({where:{uid:ctx.uid!},order:{updatedAt:'DESC' as any},take:q.limit,skip:q.offset})};}
...
export async function settingsUpdate(ctx:ActionContext,p:any){const config=(p.config??p.settings??{}) as Record<string,unknown>;await ctx.db.transaction(async(tx:EntityManager)=>{await tx.getRepository(UserSetting).upsert({uid:ctx.uid!,config},['uid']);});return settingsGet(ctx);} 

...
export async function hydrateCatalogProducts(db: DataSource, baseRows: any[]): Promise<CatalogResolvedProduct[]> {
    ...
    const productImages = productIds.length
        ? await db.query(
            `SELECT pi.productId, pi.sortOrder, ma.originalPath, ma.thumbnailPath
    ...
    const imagesByProductId = new Map<string, { imageUrl: string; thumbnailUrl: string; images: string[] }>();
    for (const row of productImages) {
        const thumbnailUrl = resolveMediaPublicUrl(row.thumbnailPath) || '';
        const imageUrl = resolveMediaPublicUrl(row.originalPath) || thumbnailUrl || '';
        const existing = imagesByProductId.get(row.productId) || { imageUrl: '', thumbnailUrl: '', images: [] };

        if (!existing.thumbnailUrl && thumbnailUrl) existing.thumbnailUrl = thumbnailUrl;
        if (!existing.imageUrl && imageUrl) existing.imageUrl = imageUrl;

        if (thumbnailUrl) existing.images.push(thumbnailUrl);
        if (imageUrl) existing.images.push(imageUrl);
    ...
    return baseRows.map((row) => {
        ...
        const rowVariants = (variantsByProductId.get(row.id) || []).map((variant) => {
            ...
            return {
                ...
                attributes,
                displayName: buildVariantDisplayName(row.name, attributes),
                options: buildVariantOptions(attributes),
            };
        });

        const defaultVariant = rowVariants[0] || null;
        const imageMeta = imagesByProductId.get(row.id) || { imageUrl: '', thumbnailUrl: '', images: [] };
...
export async function resolveStoreVisibleProducts(
    ...
    if (options.searchTerm) {
        whereParts.push('(p.name LIKE ? OR p.slug LIKE ?)');
    ...
    const normalizedRows = await hydrateCatalogProducts(db, rows);

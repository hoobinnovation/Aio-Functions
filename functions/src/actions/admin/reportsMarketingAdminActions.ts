...
function ensureSupportedFilters(filters: MarketingFilters) {
  if (filters.platform || filters.branchId || filters.deviceId) {
    throw new AppError(
        'VALIDATION_FAILED',
        'Requested filter is not supported by current attribution dataset',
        {
          unsupported: {
...
function parseGroupBy(groupBy: string[] | null | undefined): { keySql: string; keyLabel: string } {
  if (!groupBy || groupBy.length === 0) {
    return { keySql: "COALESCE(ma.campaign,'(none)')", keyLabel: 'campaign' };
  }

  if (groupBy.length > 1) {
    throw new AppError(
        'VALIDATION_FAILED',
        'Only one groupBy field is supported for marketing reports in this phase'
    );
  }
  ...
  if (!keySql) {
    throw new AppError('VALIDATION_FAILED', `Unsupported groupBy field: ${field}`);
  }

  return {
    keySql: `COALESCE(${keySql}, '(none)')`,
    keyLabel: field,
  };
}
...
function buildWhere(filters: MarketingFilters) {
  ...
  if (filters.source) {
    where.push('ma.source = ?');
    params.push(filters.source);
  }

  if (filters.medium) {
    where.push('ma.medium = ?');
    params.push(filters.medium);
  }

  if (filters.campaign) {
    where.push('ma.campaign = ?');
...
export async function adminReportsAttributionOverview(ctx: ActionContext, payload: any) {
  const q = normalizeTableQuery(
      payload,
      { sortBy: 'revenueCents', sortDir: 'desc', pageSize: 50 },
       ...
       FROM (
              ...
              FROM orders o
                ...
                FROM (
                       ...
                       JOIN marketing_attribution_events e
                            ON e.uid = x.uid AND e.createdAt = x.maxCreatedAt
              ) ma ON ma.uid = o.uid
  ...
  const attributedRevenueCents = Number(aggregateRows[0]?.attributedRevenueCents ?? 0);
  ...
  const aggregates: Record<string, unknown> = {
    ...
    topChannels: topChannelsRows.map((r: any) => ({
      channel: r.channel,
      count: Number(r.c),
    })),
    roasAvailable: false,
  ...
  if (includeRoas) {
    aggregates.roas = null;
  }

  if (includeMargin) {
    aggregates.contributionCents = null;
  }
  ...
  return {
    ...
    grouped: {
      by: [keyLabel],
      groups: items.map((r: any) => ({
        key: r.key,
        count: Number(r.ordersCount),
      })),
    },
    aggregates,
...
export async function adminReportsTopCampaigns(ctx: ActionContext, payload: any) {
  ...
  const totalRows = await ctx.db.query(
      `SELECT COUNT(*) total
       FROM (
              SELECT
                COALESCE(ma.campaign,'(none)') campaign,
                COALESCE(ma.source,'(none)') source,
                COALESCE(o.channel,'(none)') channel
              FROM orders o
                     JOIN (
                SELECT x.uid, e.source, e.medium, e.campaign, e.term, e.content
                FROM (
              ...
              WHERE o.storeId = ? AND o.createdAt BETWEEN ? AND ? ${whereSql}
              GROUP BY campaign, source, channel
            ) z`,
      [q.storeId, q.range.from, q.range.to, q.storeId, q.range.from, q.range.to, ...params]
  );
  ...
  return {
    items: pickColumns(
        items,
        ['campaign', 'source', 'channel', 'ordersCount', 'revenueCents', 'roas', 'contributionCents'],
        q.columns
    ),
    pageInfo: {
      page: q.fetchAll ? 1 : q.page,

...
import {
    normalizeTableQuery,
    applyPaginationOrFetchAll,
    applySort,
    applyGroupBySummary,
    pickColumns,
} from './reporting/tableQuery';
...
export async function reportsOverview(ctx: ActionContext, payload: any) {
    ...
    const aggregatesRows = await ctx.db.query(
        `SELECT
             ...
             COUNT(DISTINCT CASE WHEN x.cnt = 1 THEN o.uid END) newCustomersCount,
             COUNT(DISTINCT CASE WHEN x.cnt > 1 THEN o.uid END) returningCustomersCount
         FROM orders o
                  LEFT JOIN (
    ...
    if (payload?.flags?.includeProfessional) {
        ...
        const shippingFees = await ctx.db.query(
        ...
        const shippingFeesCents = Number(shippingFees[0]?.shippingFeesCents ?? 0);
...
export async function reportsOrdersByStatus(ctx: ActionContext, payload: any) {
    const q = normalizeTableQuery(
        payload,
        { sortBy: 'ordersCount', sortDir: 'desc', pageSize: 50 },
        { fallbackStoreId: ctx.storeId }
    );
    ...
    if (payload?.flags?.includeAging) {
        ...
        aggregates.aging = {
            '0_24h': Number(aging[0]?.a0 ?? 0),
            '24_48h': Number(aging[0]?.a1 ?? 0),
            '48_72h': Number(aging[0]?.a2 ?? 0),
            '72h_plus': Number(aging[0]?.a3 ?? 0),
        };
    }
    ...
    return {
        items: pickColumns(
            items,
            ['status', 'ordersCount', 'grossRevenueCents', 'netRevenueCents', 'avgOrderValueCents'],
            q.columns
        ),
        pageInfo,
        grouped,
...
export async function reportsTopProducts(ctx: ActionContext, payload: any) {
    const q = normalizeTableQuery(
        payload,
        { sortBy: 'qtySold', sortDir: 'desc', pageSize: 50 },
        { fallbackStoreId: ctx.storeId }
    );
    ...
    if (q.fetchAll && total > 10000) {
        throw new AppError('FETCH_ALL_LIMIT_EXCEEDED', 'Fetch all limit exceeded', {
            limit: 10000,
            total,
        });
    }
    ...
    const items = await ctx.db.query(
        `SELECT
             ...
             MAX(oi.nameSnapshot) name,
             MAX(pv.sku) sku,
             SUM(oi.qty) qtySold,
             COALESCE(SUM(oi.priceCents * oi.qty),0) grossRevenueCents,
             COALESCE(
                     SUM((oi.priceCents * oi.qty) - FLOOR((o.discountCents * (oi.priceCents * oi.qty)) / NULLIF(o.subtotalCents,0))),
                     0
             ) netRevenueCents
         FROM order_items oi
                  JOIN orders o ON o.id = oi.orderId
    ...
    return {
        items: pickColumns(
            items,
            ['productId', 'name', 'sku', 'qtySold', 'grossRevenueCents', 'netRevenueCents'],
            q.columns
        ),
        pageInfo: {
            page: q.fetchAll ? 1 : q.page,
            pageSize: q.fetchAll ? total : q.pageSize,
            total,
        },
        grouped,
        capabilities: capabilities(),
...
export async function reportsInventorySummary(ctx: ActionContext, payload: any) {
    const q = normalizeTableQuery(
        payload,
        { sortBy: 'name', sortDir: 'asc', pageSize: 50 },
        { fallbackStoreId: ctx.storeId }
    );

    const filters = q.filters as {
    ...
    if (filters.categoryId) {
        whereParts.push(
            '(p.categoryId=? OR EXISTS (SELECT 1 FROM product_categories pc WHERE pc.productId=p.id AND pc.categoryId=?))'
        );
        params.push(filters.categoryId, filters.categoryId);
    }

    if (filters.lowStockOnly) {
        whereParts.push('COALESCE(ib.onHandQty,0) <= 5');
    }
    ...
    const items = await ctx.db.query(
        `SELECT
             p.id productId,
             p.name name,
             COALESCE(ib.onHandQty,0) onHandQty,
             0 reservedQty,
             COALESCE(ib.onHandQty,0) availableQty,
             (COALESCE(ib.onHandQty,0) <= 5) lowStock,
             MAX(ia.createdAt) lastMovementAt
         FROM products p
                  LEFT JOIN inventory_balances ib ON ib.productId = p.id AND ib.storeId = ?
                  LEFT JOIN inventory_adjustments ia ON ia.productId = p.id
         WHERE ${whereParts.join(' AND ')}
         GROUP BY p.id, p.name, ib.onHandQty
         ORDER BY ${sortExpr} ${q.sort.dir === 'asc' ? 'ASC' : 'DESC'}
     LIMIT ? OFFSET ?`,
        [q.storeId, ...params, limit, offset]
    );

    const deadStockBoundary =
        deadStockDays > 0 ? `DATE_SUB(NOW(), INTERVAL ${deadStockDays} DAY)` : null;
    ...
    const aggr = await ctx.db.query(
        `SELECT
             ...
             SUM(
                     CASE
                         WHEN COALESCE(ib.onHandQty,0) > 0
                             ${deadStockBoundary ? `AND (MAX_IA.lastMovementAt IS NULL OR MAX_IA.lastMovementAt < ${deadStockBoundary})` : 'AND 1=0'}
           THEN 1
                         ELSE 0
                         END
             ) deadStockCount
         FROM products p
                  LEFT JOIN inventory_balances ib ON ib.productId = p.id AND ib.storeId = ?
                  LEFT JOIN (
             SELECT productId, MAX(createdAt) lastMovementAt
             FROM inventory_adjustments
             GROUP BY productId
         ) MAX_IA ON MAX_IA.productId = p.id
         WHERE ${whereParts.join(' AND ')}`,
        [q.storeId, ...params]
    );
    ...
    if (payload?.flags?.includeMovements) {
        const mov = await ctx.db.query(
            `SELECT COUNT(*) movementsCount
             FROM inventory_adjustments
             WHERE createdAt BETWEEN ? AND ?`,
            [q.range.from, q.range.to]
        );
        extraAggregates.movements = mov[0];
        availability.movements = false;
    }

    return {
        items: pickColumns(
            items,
            ['productId', 'name', 'onHandQty', 'reservedQty', 'availableQty', 'lowStock', 'lastMovementAt'],
            q.columns
        ),
        pageInfo: {
            page: q.fetchAll ? 1 : q.page,
        ...
        grouped: q.groupBy?.length ? { by: q.groupBy, groups: [] } : undefined,
        aggregates: {
            ...(aggr[0] || {}),
            ...extraAggregates,
        },
...
export async function reportsCustomersSummary(ctx: ActionContext, payload: any) {
    const q = normalizeTableQuery(
        payload,
        { sortBy: 'netRevenueCents', sortDir: 'desc', pageSize: 50 },
        { fallbackStoreId: ctx.storeId }
    );
    ...
    const items = await ctx.db.query(
        `SELECT
             o.uid customerId,
             MAX(up.displayName) name,
             COUNT(*) ordersCount,
             SUM(o.totalCents) netRevenueCents,
    ...
    const aggregates = await ctx.db.query(
        `SELECT
             COUNT(DISTINCT uid) totalCustomers,
             SUM(CASE WHEN lastOrderAt >= DATE_SUB(NOW(), INTERVAL 30 DAY) THEN 1 ELSE 0 END) activeCustomersCount
         FROM (
    ...
    if (payload?.flags?.includeCohorts) {
        const cohorts = await ctx.db.query(
            `SELECT DATE_FORMAT(firstOrderAt, '%Y-%m') cohortMonth, COUNT(*) customers
             FROM (
                      SELECT uid, MIN(createdAt) firstOrderAt
                      FROM orders
                      WHERE storeId = ?
                      GROUP BY uid
                  ) x
    ...
    if (payload?.flags?.includeLtv) {
        const ltv = await ctx.db.query(
            `SELECT
                 COALESCE(AVG(totalSpent),0) avgLtvCents,
                 COALESCE(SUM(totalSpent),0) totalLtvCents
             FROM (
                      SELECT uid, SUM(totalCents) totalSpent
                      FROM orders
                      WHERE storeId = ?
                      GROUP BY uid
                  ) x`,
        ...
        extras.ltv = ltv[0];
        availability.ltv = false;
    }

    return {
        items: pickColumns(
            items,
            ['customerId', 'name', 'ordersCount', 'netRevenueCents', 'lastOrderAt'],
            q.columns
        ),
        pageInfo: {
            page: q.fetchAll ? 1 : q.page,
...
export async function reportsReturnsSummary(ctx: ActionContext, payload: any) {
    const q = normalizeTableQuery(
        payload,
        { sortBy: 'day', sortDir: 'desc', pageSize: 50 },
        { fallbackStoreId: ctx.storeId }
    );
    ...
    const totalRows = await ctx.db.query(
        `SELECT COUNT(*) total
         FROM (
                  SELECT DATE(r.requestedAt) day, r.status reason
         FROM returns r
         WHERE r.storeId = ? AND r.requestedAt BETWEEN ? AND ?
         GROUP BY DATE(r.requestedAt), r.status
             ) t`,
        [q.storeId, q.range.from, q.range.to]
    );
    ...
    const sortExpr = sortMap[q.sort.by] ?? sortMap.day;

    const items = await ctx.db.query(
        `SELECT
             DATE(r.requestedAt) day,
             r.status reason,
             COUNT(*) returnsCount,
             COALESCE(SUM(ref.amountCents),0) refundCents
         FROM returns r
    ...
    const aggregates = await ctx.db.query(
        `SELECT
       COUNT(*) returnsCountTotal,
       COALESCE(SUM(ref.amountCents),0) refundCentsTotal
     FROM returns r
     LEFT JOIN refunds ref ON ref.returnId = r.id
     WHERE r.storeId = ? AND r.requestedAt BETWEEN ? AND ?`,
        [q.storeId, q.range.from, q.range.to]
    ...
    return {
        items: pickColumns(items, ['day', 'reason', 'returnsCount', 'refundCents'], q.columns),
        pageInfo: {
            page: q.fetchAll ? 1 : q.page,
            pageSize: q.fetchAll ? total : q.pageSize,
            total,
...
export async function reportsLoyaltySummary(ctx: ActionContext, payload: any) {
    ...
    const aggregates = await ctx.db.query(
        `SELECT
             COALESCE(SUM(CASE WHEN pointsDelta > 0 THEN pointsDelta ELSE 0 END),0) pointsIssued,
    ...
    const items = await ctx.db.query(
        `SELECT
       DATE(lt.createdAt) day,
       COALESCE(ut.name,'unknown') tier,
       COALESCE(SUM(CASE WHEN lt.pointsDelta > 0 THEN lt.pointsDelta ELSE 0 END),0) pointsEarned,
       COALESCE(SUM(CASE WHEN lt.pointsDelta < 0 THEN ABS(lt.pointsDelta) ELSE 0 END),0) redemptions
     FROM loyalty_transactions lt
     LEFT JOIN (
       SELECT uid, MAX(COALESCE(displayName,'')) name
       FROM user_profiles
       GROUP BY uid
     ) ut ON ut.uid = lt.uid
    ...
    return {
        items: pickColumns(items, ['day', 'tier', 'pointsEarned', 'redemptions'], q.columns),
        pageInfo: {
            page: q.fetchAll ? 1 : q.page,
            pageSize: q.fetchAll ? items.length : q.pageSize,
            total: items.length,
        },
        grouped: q.groupBy?.length ? { by: q.groupBy, groups: [] } : undefined,
...
export async function reportsCashbackSummary(ctx: ActionContext, payload: any) {
    ...
    return {
        items: pickColumns(
            items,
            ['day', 'campaignId', 'cashbackIssuedCents', 'cashbackRedeemedCents'],
            q.columns
        ),
        pageInfo: {
            page: q.fetchAll ? 1 : q.page,

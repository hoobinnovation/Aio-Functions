...
export function normalizeTableQuery(
  ...
  if (!storeId) {
    throw new AppError('VALIDATION_FAILED', 'storeId is required');
  }
...
export function sanitizeGroupBy(groupBy: string[] | null, allowedGroupFields: string[]): string[] {
  ...
  if (invalid.length) {
    throw new AppError('VALIDATION_FAILED', 'Invalid groupBy fields', { invalid });
  }
  return groupBy;
}
...
export function sanitizeColumns(columns: string[] | null, allowedColumns: string[]): string[] | null {
  ...
  if (invalid.length) {
    throw new AppError('VALIDATION_FAILED', 'Invalid columns', { invalid });
  }
  return columns;
}
  ...
  if (typeof arg1 === 'number') {
    ...
    if (query.fetchAll) {
      ...
      return {
        limit: MAX_FETCH_ALL,
        offset: 0,
        pageInfo: { page: 1, pageSize: total, total },
      };
    }
...
export async function applyGroupBySummary(
    qbBase: any,
    query: TableQuery,
    allowedGroupFields: Record<string, string>
) {
  ...
  const rows = await qbBase
      .clone()
      .select(`CONCAT(${expr})`, 'groupKey')
      .addSelect('COUNT(*)', 'count')
      .groupBy(groups.map((g) => allowedGroupFields[g]).join(', '))
      .getRawMany();
...
export async function buildGroupedSummary(
  ...
  const rows = await db.query(
      `SELECT CONCAT(${expr}) key, COUNT(*) count ${baseFromWhereSql} GROUP BY ${groups
          .map((g) => groupExprMap[g])
          .join(', ')}`,
      params
  );

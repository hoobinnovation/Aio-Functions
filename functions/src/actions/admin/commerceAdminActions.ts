...
const crud=(entity:any,name:string)=>({
  list: async (ctx:ActionContext,p:any={})=>{const q=normalizeListQueryInput(p,{defaultPageSize:50,maxPageSize:200});const storeId=resolveStoreScopedId(ctx.storeId,p.storeId);return { items: await ctx.db.getRepository(entity).find({where:{storeId},order:{id:'DESC' as any},take:q.limit,skip:q.offset})};},
...
export async function adminDeliveryZonesList(ctx:ActionContext,p:any={}){const q=normalizeListQueryInput(p,{defaultPageSize:50,maxPageSize:200});const storeId=resolveStoreScopedId(ctx.storeId,p.storeId);const zones=await ctx.db.getRepository(DeliveryZone).find({where:{storeId},order:{id:'DESC' as any},take:q.limit,skip:q.offset});const gov=await ctx.db.getRepository(Governorate).find();return {zones,governorates:gov};}
...
export async function adminNotificationsList(ctx:ActionContext,p:any={}){const q=normalizeListQueryInput(p,{defaultPageSize:100,maxPageSize:200});return {notifications:await ctx.db.query('SELECT * FROM notifications ORDER BY createdAt DESC LIMIT ? OFFSET ?', [q.limit,q.offset])};}

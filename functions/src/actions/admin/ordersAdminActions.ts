...
export async function adminOrdersList(ctx:ActionContext,p:any){return {orders:await ctx.db.getRepository(Order).find({where:{storeId:p.storeId},order:{createdAt:'DESC' as any}})};}
...
export async function adminRiskFlaggedOrdersList(ctx:ActionContext,p:any){return {flags:await ctx.db.getRepository(RiskFlag).find({where:{status:'flagged'},take:p.limit||100})};}
...
export async function adminReturnsList(ctx:ActionContext,p:any){return {returns:await ctx.db.getRepository(Return).find({where:{storeId:p.storeId},order:{requestedAt:'DESC' as any}})};}

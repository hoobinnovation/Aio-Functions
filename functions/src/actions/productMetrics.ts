...
export async function recomputeProductRatingMetrics(tx: EntityManager, productId: string) {
  const [row] = await tx.getRepository(Product).query(
    'SELECT COUNT(*) ratingCount, COALESCE(AVG(rating), 0) ratingAverage FROM order_reviews WHERE productId=?',
    [productId],
  );
...
export async function recomputeProductPopularityMetrics(tx: EntityManager, productId: string, ratingAverage: number, ratingCount: number) {
  ...
  const [ordersRow] = await tx.getRepository(Product).query(
    `SELECT COALESCE(SUM(oi.qty),0) completedOrderQty
     ...
     WHERE oi.productId=? AND o.paymentStatus='paid' AND o.status NOT IN (${EXCLUDED_ORDER_STATUSES.map(() => '?').join(',')})`,
    [productId, ...EXCLUDED_ORDER_STATUSES],
  );
...
export async function recomputeProductMetrics(tx: EntityManager, productId: string) {
  const rating = await recomputeProductRatingMetrics(tx, productId);
  const popularity = await recomputeProductPopularityMetrics(tx, productId, rating.ratingAverage, rating.ratingCount);
...
export async function recomputeStoreProductsMetrics(tx: EntityManager, storeId: string) {
  const rows = await tx.getRepository(Product).find({ where: { storeId }, select: ['id'] });
  for (const row of rows) {
    await recomputeProductMetrics(tx, row.id);
  }

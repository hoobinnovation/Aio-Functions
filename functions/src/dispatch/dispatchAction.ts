...
function toErrorResponse(err: unknown, requestId: string, serverTime: string): UnifiedResponse {
  const rawCode = typeof err === 'object' && err !== null && 'code' in err ? String((err as { code: unknown }).code) : undefined;
...
export async function dispatchAction(gateway: Gateway, request: UnifiedRequest, ctx: RequestContext): Promise<UnifiedResponse> {
  try {
